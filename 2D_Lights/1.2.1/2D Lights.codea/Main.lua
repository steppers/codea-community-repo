...
function setup()
  ...
  buddah.shader.LightsAmbientColor = vec4(0.6, 0.6, 1, 0.2) -- ambient RGBA -- alpha is intensity 
  ...
  rocks.shader.LightsAmbientColor = vec4(0.6, 0.6, 1, 0.2) -- ambient RGBA -- alpha is intensity 
  ...
  f = [[
    ...
    vec4 getLight(vec4 diffuse, vec3 normal, vec3 pos, vec4 color, float size, bool relative, vec3 falloff, float cutoff) {
      ...
      if (cutoff > 0.0) {     
         ...
         if (rad > cutoff) {
            lightsColor = vec4(diffuse.rgb * ambient, diffuse.a);
         }
      }
      return lightsColor;
    }
