...
function demo_imports()
    ...
    local src_main = ASSource('main', [[
        ...
        declare function getLuaVersionInline(): string;
        declare const IMPORT_VAL: u32;
        
        export function main(): void {
            print(`[demo_imports] Lua: ${getLuaVersion()}`);
            print(`[demo_imports] Lua Inlined: ${getLuaVersionInline()}`);
            print(`[demo_imports] Imported val: ${IMPORT_VAL}`);
        }
    
        // Inline JS imports!
        @js_import getLuaVersionInline =
            () => codea.lua_version
        @js_end
    ]],
    {
        ...
        -- Or basic values directly (not strings)
        ["IMPORT_VAL"] = 970932,
    })
