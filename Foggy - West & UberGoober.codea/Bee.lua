...
function Bee:draw()
    ...
    for i=1,#self.old do
      ...
      --  sprite("Project:foggybodymask",0,0,self.size*(30-i)/30)
        sprite(spriteTable[19],0,0,self.size*(30-i)/30)
        noTint()
        popMatrix()
    end
    pushMatrix()
    translate(self.x,self.y)
   -- sprite("Project:foggyhead",0,0,self.size)
    sprite(spriteTable[3],0,0,self.size) 
    rotate(self.bodyangle+self.bodyoffsetangle)
    
    
    
    if self.injured==0 then
       -- sprite("Project:foggybody",0,0,self.size)
        sprite(spriteTable[2],0,0,self.size)
    else
       -- sprite("Project:foggybodydamage",0,0,self.size)
        sprite(spriteTable[17],0,0,self.size)
 ...
 --   sprite("Project:foggywing",0,0,self.size)
    sprite(spriteTable[5],0,0,self.size)
 ...
 --   sprite("Project:foggylegs",0,0,self.size)
    sprite(spriteTable[6],0,0,self.size)
    
    rotate(-self.legsangle-self.bodyoffsetangle)
    rotate(self.legmangle+self.bodyoffsetangle)
   -- sprite("Project:foggylegm",0,0,self.size)
    sprite(spriteTable[1],0,0,self.size)
    rotate(-self.legmangle-self.bodyoffsetangle)
    rotate((self.bodyangle+self.bodyoffsetangle)*1.5)
  --  translate(-0.1*self.size,-0.09*self.size)
  --  sprite("Project:foggylegl",0,0,self.size)
    sprite(spriteTable[4],0,0,self.size)
    rotate(-(self.bodyangle+self.bodyoffsetangle)*1.5)
    popMatrix()
    
    if self.invincible>0 then
        --sparkles
        --    table.insert(particles,Particle(self.x,self.y,math.random(360),14,math.random(5),-90+math.random(360),math.random(3),0,math.random(30)))
        table.insert(particles,Particle(self.x,self.y,0,15,0,0,0,0,10))
        
    end
    
    if self.shield==1 then
        tint(255,255,255,100)
      --  sprite("Project:fbubble",self.x-5,self.y-5,100,100)
        sprite(spriteTable[10],self.x-5,self.y-5,100,100)
        noTint()
    end
    
    
    if math.fmod(counter,self.bulletspeed)==1 and self.drag==0 then
   --    sound(SOUND_SHOOT, 19478,7)
...
--    sound("Game Sounds One:Blaster",1.0,pitch,0.0,false)        
    ...
    --    sound("Game Sounds One:Blaster",1.0,1.2,0.0,false)
    end
    if self.bombrate>=2 and math.fmod(counter,self.bulletspeed*10)==1 and self.drag==0 then
        ...
        sound(SOUND_SHOOT, 46595)        
   --     sound("Game Sounds One:Blaster",1.0,1.0,0.0,false)
    end
    if self.bombrate>=3 and math.fmod(counter+10,self.bulletspeed*10)==1 and self.drag==0 then
        ...
        sound(SOUND_SHOOT, 46595)        
  --      sound("Game Sounds One:Blaster",1.0,1.4,0.0,false)
    end
