...
function processGame()
    ...
    if Global.playerFired then
        sound(asset.documents.Galaxian_assets.playerfired)
    end
end
...
function processPlayerDied()
    if gameCount > gameResume and not attacking then
        ...
        if numPlayerLives < 0 then
            gameState = Global.GSTATE_GAMEOVER
            sound(asset.documents.Galaxian_assets.gameover)
        else
...
function processChangingLevel()
    if gameState == Global.GSTATE_CHANGINGLEVEL1 then
        if gameCount > gameResume then
            ...
            sound(asset.documents.Galaxian_assets.getready)
        else
            animate()
        end
    elseif gameState == Global.GSTATE_CHANGINGLEVEL2 then
        if gameCount > gameResume then
            ...
            initBadGuyGrid()
            gameState = Global.GSTATE_PLAYING
            sound(asset.documents.Galaxian_assets.startgame)
        else
            updateBackground()
        end
    end
                ...
                if s2.type ~= 0 and s1 ~= s2 and differentSigns and s2.alive then
                    ...
                    if (s1:getCollisionRect():intersects(s2:getCollisionRect())) then
                        ...
                        elseif ctype == Global.COL_EMISSILE_PLAYER then
                            ...
                            sound(asset.documents.Galaxian_assets.playerexplode)
                        elseif ctype == Global.COL_PLAYER_BADGUY then
                            ...
                            if s1.type == Global.HEADBADGUY_TYPE or s2.type == Global.HEADBADGUY_TYPE then
                                sound(asset.documents.Galaxian_assets.shotbigbadguy)
                            else
                                if (math.random(100) > 50) then
                                    sound(asset.documents.Galaxian_assets.shotbadguy1)
                                else
                                    sound(asset.documents.Galaxian_assets.shotbadguy2)
                                end
                            end
                            sound(asset.documents.Galaxian_assets.playerexplode)
        ...
        for row=1, 6, 1 do
            if grid[col][row] ~= nil then
                ...
                elseif grid[col][row].classType == "BadGuySprite" then
                    ...
                    if math.random(0) < enemyFireFrequency and s:isInChaseMode() then
                        ...
                        sound(asset.documents.Galaxian_assets.enemyfired)
                    end
                end
                doAttack = false
            ...
            if (doAttack) then
                if not s:isAttacking() then
                    sound(asset.documents.Galaxian_assets.badguyattacking)
                    s:attack(dir)
...
function loadMedia()
    ...
    -- extra life
    extraLife = readImage(asset.documents.Galaxian_assets.extralife0)
    -- level flags
    levelFlags[Global.LEVELFLAG_NORMAL] = readImage(asset.documents.Galaxian_assets .. Global.LEVELFLAG_IMAGE_BASE .. "0")
    levelFlags[Global.LEVELFLAG_WORTH5] = readImage(asset.documents.Galaxian_assets .. Global.LEVELFLAG_IMAGE_BASE .. "1")
end
...
function loadImages(n, base)
    ...
    local img
    for i = 0, n, 1 do
        img = readImage(asset.documents.Galaxian_assets .. base .. i)
        table.insert(sf, img)
    end
    return sf
end
