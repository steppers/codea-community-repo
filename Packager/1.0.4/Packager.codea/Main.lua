...
function setup()
    ...
    :add_children(
        ...
        Oil.TextButton(0.5, 0, 200, 30, "Compile", function(bttn)
            ...
            tween(0.1, {}, {}, nil, function()
                Packager.CompileProject(project_selected, strip)
                bttn:add_style("text", "Compile")
            end)
        end),
    
        Oil.TextButton(0.5, 0, 200, 30, "Package dependencies", function(bttn)
            -- Check a project is actually selected
            if project_selected == nil then
                bttn:add_style("text", "Select a project!")
                tween(1.2, {}, {}, nil, function()
                    bttn:add_style("text", "Package dependencies")
                end)
                return
            end
        
            -- Package the project
            bttn:add_style("text", "Packaging...")
            tween(0.1, {}, {}, nil, function()
                Packager.IncludeDependencies(project_selected)
                bttn:add_style("text", "Package dependencies")
            end)
        end)
    )
