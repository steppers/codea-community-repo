...
function draw()
    ...
    local cornerOffset = WIDTH*0.035
    
    if gamemode==1 then
        pushStyle()
        textMode(CORNER)
        textAlign(LEFT)
        text("People eaten: "..50-#men,cornerOffset,cornerOffset)
        local hiScoreText = "Hi-score: "..hiscore
        local rightOffset, heightOffset = textSize(hiScoreText)
        textAlign(RIGHT)
        text(hiScoreText,WIDTH-rightOffset-cornerOffset,HEIGHT-cornerOffset-heightOffset)
        textAlign(LEFT)
        text("Score: "..score,cornerOffset,HEIGHT-cornerOffset-heightOffset)
        popStyle()
    else
        pushStyle()
        textMode(CORNER)
        textAlign(LEFT)
        text("People eaten: "..p1count,cornerOffset,cornerOffset)
        local p2Text = "People eaten: "..p2count
        local rightOffset = textSize(p2Text)
        text("People eaten: "..p2count,WIDTH-rightOffset-cornerOffset,cornerOffset)
        popStyle()
    end
    ...
    text(math.floor(counter),WIDTH/2,HEIGHT*0.92)
    fontSize(16)
    if #men==0 then
        if gamestate~=GAMEOVER then
            score = score + 250
    ...
    elseif gamestate==READY then
        ...
        playerxspd=0
        local titleSize, readySize, infoSize = HEIGHT * 0.07, HEIGHT * 0.035, HEIGHT * 0.0265
        print(titleSize)
        font("ArialRoundedMTBold")
        fill(220, 194, 30, 255)
        strokeWidth(3)
        ...
        fontSize(readySize)
        text("Ready?",WIDTH/2,HEIGHT*0.25)
        textAlign(CENTER)
        ...
        fill(200,255,255)
        text(gamemode.." Player Mode",WIDTH/2,cornerOffset*4)
