...
function Soda.Frame:setPosition() --all elements defined relative to their parents. This is recalculated when orientation changes
    ...
    if t.fullscreen then
        self.x, self.w = Soda.parseCoordSize(0.5 / 1.2, 1.0, WIDTH * 1.2)
        self.y, self.h = Soda.parseCoordSize(0.5 / 1.2, 1.0, HEIGHT * 1.2)
    else
...
function Soda.Frame:draw(breakPoint)
    ...
    if self.shape and not self.texture then
        self:drawShape(sty)
    elseif self.texture then
        sprite(self.texture, self.w * 0.5, self.h * 0.5, self.w, self.h)
    end
        popStyle()
    pushStyle()
