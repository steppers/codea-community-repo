...
function main()
    ...
    webview:loadJS("https://cdnjs.cloudflare.com/ajax/libs/require.js/2.3.6/require.min.js")
...
function WebView:init(htmlAsset)
    ...
    -- Remove any old WKWebView view to free memory
...
local function loadJSFromURL(view, url, async)
    local promise = ST.Promise()
    
    runMain(function()
        http.request(url, function(data, status)
            promise:resolve(data)
        end, function(err)
            print("Failed to download JS from URL: " .. url)
            ST.hang()
        end)
    end)
    
    loadJSFromSource(view, promise:get(), async)
end
