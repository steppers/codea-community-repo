...
-- Auto import console.log, console.warning, console.error
-- Better error output during call() & callAsync()
...
local function importInternals(webview)
    ...
    webview:import("console.warning", function(...)
        objc.warning(...)
    end)
    webview:import("console.error", function(...)
        objc.warning(...)
        ST.abort()
    end)
    
    -- Override the Worker() constructor function to work correctly
    -- in a WKWebView
    webview:loadJS([[
        const __wkwebview_orig_Worker_func = window.Worker;
        window.Worker = function(aURL, options) {
        
            // Load the synchronously first
            ...
            if (request.status === 200) {
                ...
                return new __wkwebview_orig_Worker_func(window.URL.createObjectURL(blob), options);
            }
        
            // TODO: Reproduce exceptions
        }
    ]], true)
end
    ...
    runMain(function()
        self.view:callAsyncJavaScript_arguments_inFrame_inContentWorld_completionHandler_(
            ...
            function(objResult, objError)
                if objError then
                    local info = objError.userInfo
                    if info.WKJavaScriptExceptionMessage == nil then
                        for k,v in pairs(info) do
                            objc.warning(k .. ": " .. v)
                        end
                    else
                        objc.warning(info.WKJavaScriptExceptionMessage);
                    end
                end
