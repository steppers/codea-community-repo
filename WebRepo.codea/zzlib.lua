...
function zzlib.unzip(buf,filename)
    ...
    --
    -- This could be improved by checking that the comment length matches
    -- but I can't be bothered at the moment. That'll bite me...
    while int4le(buf,p) ~= 0x06054b50 do
        p = p - 1
    end
    ...
    for i=1,nfiles do
        ...
        if name == filename then
            ...
            if method == 0 then
                ...
                -- DEFLATE compression
                local bs = inflate.bitstream_init(buf)
                bs.pos = p
                result = arraytostr(inflate.main(bs))
            end
