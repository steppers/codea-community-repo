...
function setup()
    ...
    :add_child(
        ...
        :add_children(
            ...
            Oil.IconButton(0.5, 0, 150, 50, asset.builtin.Blocks.Dirt_Grass),
...
function Oil.TextEntry(x, y, w, h, default_text, callback)
    ...
    node:add_handler(function(_, event)
        ...
        -- 
        if node.state.focus and event.type ~= "hover" and event.pos and not node:covers(event.pos) then
            print("defocus", event.type)
            hideKeyboard()
        ...
        if node.state.focus and (event.type == "key" or event.type == "press") then
            local str = node:get_style("text")
            ...
            if event.type == "key" then
                if event.key == BACKSPACE then
                    ...
                    -- Move cursor along
                    cursor_index = cursor_index + string.len(event.key)
                end
            elseif pasteboard.text ~= nil then -- press event
                -- Paste
                node:set_style("text", str:sub(0,cursor_index-1) .. pasteboard.text .. str:sub(cursor_index, -1))
                    
                -- Move cursor along
                cursor_index = cursor_index + string.len(pasteboard.text)
            end
...
function Oil.StatusModal(msg, style_sheet)
    local root = Oil.Rect(0, 0, 1.0, 1.0)
    :set_priority(100) -- over everything
    :add_style("fill", color(0, 128)) --darken the background
