...
function ZipArchive:init(path, openForWrite)
    self.file = io.open(path, (openForWrite and "wb") or "rb")
    self.cd = {}
    self.prefix = ""
...
function ZipArchive:readFile(path)
    if not self.readOnly then
        error("Archive is opened writing!")
    end
...
function ZipArchive:addFile(file_asset)
...
function ZipArchive:addFolder(folder_asset)
