...
function AppWindow(app)
    ...
    -- Desc. label
    local desc = Oil.Label(116, -286, 1.0, 40, app.info.short_description, LEFT)
    ...
    -- Update Notes
    local update_notes_label = Oil.LabelResize(10, -0.0001, -10, 10, "", LEFT)
        :set_style_sheet(UI.THEME.news_internal)
        :add_style({
            fontSize = 16,
    ...
    -- Version dropdown
    local version_dd = Oil.Dropdown(-10, -332, 120, 30, app.version)
    :add_style(UI.THEME.dropdown)
    
    local function update_button_text(bttn)
        if not DB.isAppDownloaded(app.name, version_dd:get_style("text")) then
            bttn:add_style("text", "DOWNLOAD")
        elseif not DB.isAppInstalled(app.name, version_dd:get_style("text")) then
    ...
    local button = Oil.TextButton(116, -332, 150, 30, "GET", function(bttn)
        ...
        if bttn:get_style("text") == "DOWNLOAD" then
            bttn:set_style("text", "DOWNLOADING")
    ...
    for i = #DB.apps[app.name], 1, -1 do
        ...
        update_notes = update_notes .. app.version .. ":\n" .. app.info.update_notes .. "\n\n"
    ...
    update_notes_label:add_style("text", update_notes)
