...
function DB.installApp(name, version)
    ...
    if manifest then
        ...
        for path in manifest:gmatch("(.-)\n") do
            ...
            path = path:match("(%.codea/.*)")
            
            if path:find("/.-/") then -- 2 slashes (subfolder)
                local sf = (asset.documents).path .. "/" .. project_name .. path:gsub("/[^/]-$", "")
            ...
            local file = io.open((asset.documents).path .. "/" .. project_name .. path, "wb")
            file:write(data)
            file:close()
        end
    else
