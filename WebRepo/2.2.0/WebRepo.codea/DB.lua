...
function DB.retrieveFile(path, callback, force)
    ...
    -- TODO: Pipe directly to a file
    http.request("https://" .. _BACKEND_IP_ .. ":8081/" .. PrepareURL(path), success, fail)
end
...
function DB.installApp(name, version)
    ...
    print(DB.fs.vfs_root .. DB.fs:getFile(string.format("%s/%s/project.zip", name, version)).file)
...
function DB.getAppIcon(app, callback)
    ...
    DB.retrieveFile(string.format("%s/Icon.png", app.name), function(data)
        if data then
            data = image(data)
        end
        callback(data)
...
function DB.approveReviewCandidate(queue_entry, review_key, callback)
    ...
    :then_do(function(cb)
        ...
        http.request("https://" .. _BACKEND_IP_ .. ":8081/approve", function(response, code)
            if code == 200 then
...
function DB.rejectReviewCandidate(queue_entry, review_key, callback)
    ...
    Oil.Alert(
        ...
        function(result)
            if result then
                http.request("https://" .. _BACKEND_IP_ .. ":8081/reject", function(response, code)
                    if code == 200 then
